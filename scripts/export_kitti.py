...
class KittiConverter:
    ...
    def nuscenes_gt_to_kitti(self,
        ...
        with parallel_backend('threading', n_jobs=self.parallel_n_jobs):
            Parallel()(
                delayed(self.process_token_to_kitti)(idx, sample_token)
                for idx, sample_token
                in enumerate(tqdm(sample_tokens))
            )

        # save index to sample token mapping in a json file
        idx2token = {'%06d' % idx: token for idx, token in enumerate(sample_tokens)}
        idx2token_path = str(self.store_dir / 'idx2token.json')
        with open(idx2token_path, 'w') as f:
            json.dump(idx2token, f, indent=2)

    def process_token_to_kitti(self, token_idx: int, sample_token: str) -> None:
        token_idx = '%06d' % token_idx # Alternative to use KITTI names.
        # Get sample data.
