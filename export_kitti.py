...
the image/lidar/calib/label are saved according to sample token
...
-  (): Render the annotations of the (generated or real) KITTI dataset.
...
- python export_kitti.py render_kitti --nusc_kitti_dir ./data/nusc_kitti/mini/KITTI/object/ --render_2d False
...
To work with the original KITTI dataset, use these parameters:
 ...
 ??? what does above two lines mean? there's no `training` split available!
...
class KittiConverter:
    def __init__(self,
                 nusc_data_root: str = './data/nuscenes/v1.0-mini/',
        ...
        self.nusc = NuScenes(dataroot=nusc_data_root, version=nusc_version)

        self.image_count = len(self.nusc.sample) # number of samples in the dataset
        self.idx2token = {}
    ...
    def nuscenes_gt_to_kitti(self) -> None:
        ...
        '''modified below function, no more train/val in diff folders, all in one now'''
        ...
        tokens = []
        for token_idx, sample_token in enumerate(sample_tokens):

            self.idx2token[token_idx] = sample_token
            token = '%06d' % token_idx # Alternative to use KITTI names.
            #token = sample_token # If you wanna save files by their sample token name
            ...
            print('%d, Done with %s' % (token_idx, token))

        df = pd.DataFrame(self.idx2token.items(), columns=['index', 'sample_token'])
        df.to_csv(os.path.join(self.nusc.dataroot, 'idx2token.csv'), index=False)
